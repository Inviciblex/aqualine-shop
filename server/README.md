...
| `ADMIN_TOKEN`    | Секрет для входа в админку (`admin.html`). Пусто → выключена   |
...
включает и меняет в `admin.html` — текст хранится в БД (таблица `settings`) и
...
заголовок `X-Admin-Token`. Дополнительно можно прикрыть `admin.html` базовой
